...
export function WritingsProvider({ children }: { children: ReactNode }) {
  ...
  // Uses local-first strategy: returns cached/local version instantly when available,
  // only fetches from Firebase when body isn't cached or local version is out of sync
  const getFullWriting = useCallback(async (id: string): Promise<Writing | null> => {
    ...
    if (writing) {
      ...
      if (!writing.isSynced && navigator.onLine) {
        ...
        if (firebaseWriting) {
          ...
          const remoteTime = new Date(firebaseWriting.updatedAt).getTime();

          if (remoteTime > localTime) {
            // Remote is newer - use Firebase version
    ...
    // 3. Not cached locally - fetch from Firebase
    if (navigator.onLine) {
